...
<div id="content"><?php echo $content_top; ?>
  ...
  <table class="compare-info">
    ...
    <tbody>
	  ...
	  <tr>
        ...
        <?php foreach ($products as $product) { ?>
        <td><?php echo $products[$product['product_id']]['minimum']; ?></td>
        <?php } ?>
      </tr>
      <!--<tr>
      ...
      <tr>
        <?php foreach ($products as $product) { ?>
          <?php if ($products[$product['product_id']]['weight']) { ?>
      ...
      <!--<tr>
        ...
        <?php foreach ($products as $product) { ?>
        <td><?php echo $products[$product['product_id']]['upc']; ?></td>
        <?php } ?>
      </tr>-->
    </tbody>
    ...
    <thead>
      <tr>
        <td class="compare-attribute" colspan="<?php echo count($products) + 1; ?>">Характеристики</td>
      </tr>
    </thead>
    <tbody>
    <?php $isNoUPC = true; ?> 
    <?php foreach ($option_groups as $key => $option) { ?>
      <tr>
        ...
        <?php foreach ($products as $product) { ?>
          <td>
          <?php foreach ($option['options'] as $key => $option_value) { ?>
            <?php if (isset($products[$product['product_id']]['options'][$key])) { ?>
            <?php echo $products[$product['product_id']]['options'][$key][0]['name']; ?> (<?php echo $products[$product['product_id']]['options'][$key][0]['price']; ?>)<br>
            <?php if ($option['name'] == $text_upc) { $isNoUPC = false; } ?>
            <?php } else if ($option['name'] == $text_upc) {?>
            <?php echo $products[$product['product_id']]['upc'];?> (<?php echo $products[$product['product_id']]['price'];?>)<?php break;?>
            <?php }?>
          <?php } ?>
          </td>
        <?php } ?>
    ...
    <?php } ?>
    <?php if ($isNoUPC && $products[0]['upc'] != "") { ?>
      <tr>
        <td><?php echo $text_upc; ?></td>
        <?php foreach ($products as $product) { ?>
          <td><?php echo $products[$product['product_id']]['upc'];?> (<?php echo $products[$product['product_id']]['price'];?>)</td>
        <?php } ?>
      </tr>
    <?php } else { ?>
      <script type="text/javascript">
        $('.compare-attribute').remove();
      </script>
    <?php } ?>
    </tbody>
    <tr>
  ...
  <div class="buttons">
    <div class="right"><a href="javascript:window.history.back();" class="button"><?php echo $button_continue; ?></a></div>
  </div>
  <?php } else { ?>
