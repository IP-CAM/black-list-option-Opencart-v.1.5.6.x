...
class ModelCatalogProduct extends Model {
	...
	public function getProduct($product_id) {
		...
		if ((int)$this->config->get('config_language_id') == 7) {
			$sku = "pcs.";
		} else {
			$sku = "шт.";
		}
		...
		if ($query->num_rows) {
			return array(
				...
				'images_option'	   => $query->row['images_option'],
...
SELECT a.attribute_id, ad.name, pa.text, pa.check_box 
...
ORDER BY a.sort_order, ad.name");
		...
		foreach ($product_option_query->rows as $product_option) {
			if ($product_option['type'] == 'select' || $product_option['type'] == 'radio' || $product_option['type'] == 'checkbox' || $product_option['type'] == 'image') {
				...
				foreach ($product_option_value_query->rows as $product_option_value) {
                    if($product_option_value['option_value_id'] != 205 && $product_option_value['option_value_id'] != 206 && $product_option_value['option_value_id'] != 207){
                        $product_option_value_data[] = array(
	...
	public function getTotalProductSpecials() {
		...
		//added
	public function getMain($product_id) {
		$query = $this->db->query("SELECT category_id FROM " . DB_PREFIX . "product_to_category WHERE main_category = '1' AND product_id = '" . (int)$product_id . "'");
