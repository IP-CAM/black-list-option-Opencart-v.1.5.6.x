...
class ControllerPaymentPaymentSchet extends Controller {
    ...
    public function printpay() {
		...
		$this->data['text_preambula'] = $this->language->get('text_preambula');		
		$this->data['text_buchgalter'] = $this->language->get('text_buchgalter');
		$this->data['text_director'] = $this->language->get('text_director');
		$this->data['button_confirm'] = $this->language->get('button_confirm');
		$this->data['button_back'] = $this->language->get('button_back');
		$this->data['text_counts'] = $this->language->get('text_counts');
		$this->data['text_counts2'] = $this->language->get('text_counts2');
		$this->data['bank'] = $this->config->get('payment_schet_bank');
        ...
        $pdv= $this->config->get('payment_schet_inn');
        $totalbezpdv =  (($order_info['total'])/(100+$pdv))*100;
        $totalpdv = $order_info['total'] - $totalbezpdv;

        $this->data['totalbezpdv'] = $this->currency->format($totalbezpdv, $order_info['currency_code']);
        $this->data['totalpdv'] = $this->currency->format($totalpdv, $order_info['currency_code']);
        $this->data['totalzpdv'] = $this->currency->format($order_info['total'], $order_info['currency_code']);
		...
		$this->data['price'] = $this->model_tool_suminwords->num2str($this->data['totalzpdv']);
		...
		if (!$order_info['payment_address_2']) {
			$this->data['address'] = $order_info['shipping_method'] . ' ' . $order_info['payment_address_1'] . '<br/> ' . $order_info['payment_postcode'] . ' ' . $order_info['payment_zone'] . ' ' . $order_info['payment_city'] . '<br/> ' .$order_info['payment_country'] ;
		} else {
			$this->data['address'] = $order_info['shipping_method'] . ' ' . $order_info['payment_address_1'] . '<br/> ' . $order_info['payment_postcode'] . ' ' . $order_info['payment_zone'] . ' ' . $order_info['payment_city'] . '<br/> ' .$order_info['payment_country'] ;
		}
		...
		if ((!$this->cart->hasProducts() && empty($this->session->data['vouchers'])) || (!$this->cart->hasStock() && !$this->config->get('config_stock_checkout'))) {
			$redirect = $this->url->link('checkout/cart');
				...
				$options = $this->model_account_order->getOrderOptions($this->request->get['order_id'], $product['order_product_id']);
                    ...
                    $pricewpdv = (($product['price'])/(100+$pdv))*100;
                    $totalwpdv = (($product['total'])/(100+$pdv))*100;
					...
					$this->data['products'][] = array(
						...
						'pricewpdv'    => $this->currency->format($pricewpdv + ($this->config->get('config_tax') ? $product['tax'] : 0), $order_info['currency_code'], $order_info['currency_value']),
						'clearpricewpdv'    => $pricewpdv,
						'total'    => $this->currency->format($product['total'] + ($this->config->get('config_tax') ? ($product['tax'] * $product['quantity']) : 0), $order_info['currency_code'], $order_info['currency_value']),
						'totalwpdv'    => $this->currency->format($totalwpdv + ($this->config->get('config_tax') ? ($product['tax'] * $product['quantity']) : 0), $order_info['currency_code'], $order_info['currency_value']),
...
public function confirm() {
		...
		$comment .= str_replace('{href}', $this->url->link('payment/payment_schet/printpay', 'order_id=' . $order_info['order_id'], 'SSL'), $this->language->get('text_printpay'));
		$this->model_checkout_order->confirm($order_info['order_id'], $this->config->get('payment_schet_order_status_id'), $comment, true);
		}
	}
}
?>