	...
	protected function index($setting) {
		if($setting['type'] == 1) {
			...
			if(isset($this->request->get['limit'])) {
				$url .= '&limit=' . $this->request->get['limit']."Fuck";
			}
...
//            echo '</pre>';
	...
	public function getProducts() {
		...
		$pages = (int)($product_total / $limit) + (($product_total % $limit > 0) ? 1 : 0);

		$json = json_encode(array('result_html' => $result_html, 'min_price' => $min_price, 'max_price' => $max_price, 'page' => $page, 'pages' => $pages, 'pagination' => $pagination->render(),
