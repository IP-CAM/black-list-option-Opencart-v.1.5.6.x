...
class ControllerCommonSeoPro extends Controller {
	...
	public function index() {
		...
		if (!isset($this->request->get['_route_'])) {
			...
			foreach ($parts as $keyword) {
				if($keyword == "ua" || $keyword == "en"){
					$rows[] = array('keyword' => $keyword, 'query' => "common/home");
				}
			...
			if (isset($this->request->get['product_id'])) {
				...
				if (isset($parts[1]) && isset($queries[$parts[1]]) && ($parts[0] == "ua" || $parts[0] == "en")) {
					$this->request->get['route'] = $queries[$parts[1]];
				}elseif (isset($queries[$parts[0]])) {
	...
	public function rewrite($link) {
		...
		if($this->session->data['language'] != "ru"){
			$seo_url .= $this->session->data['language'];
		}
	...
	private function validate() {
		if (empty($this->request->get['route']) || $this->request->get['route'] == 'error/not_found') {
			if($this->session->data['language'] != "ru"){
			$this->response->redirect($this->config->get('config_url').$this->session->data['language']);
			}
			return;
		}
