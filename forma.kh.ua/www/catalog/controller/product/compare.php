			...
			if ($product_info) {
				...
				foreach ($options as $option) {
					if ($option["type"] == "select") {
						$this->data['option_groups'][$option['option_id']]['name'] = $option['name'];

						foreach ($option['option_value'] as $option_value) {
							$this->data['option_groups'][$option['option_id']]['options'][$option_value['option_value_id']]['name'] = $option_value['name'];
							if ($option_value['price_prefix'] == "+") {
								$option_price = floatval($product_info['price']) + floatval($option_value['price']);
							} else if ($option_value['price_prefix'] == "-") {
								$option_price = floatval($product_info['price']) - floatval($option_value['price']);
							}
							$options_data[$option_value['option_value_id']][] = array(
								'name' => $option_value['name'],
								'price' => $this->currency->format($this->tax->calculate($option_price, $product_info['tax_class_id'], $this->config->get('config_tax')))
							);
						}
					}
				}
				...
				if (intval($product_info['weight']) === 0) {
					$weight = false;
				} else {
