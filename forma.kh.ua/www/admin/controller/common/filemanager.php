...
class ControllerCommonFileManager extends Controller {
	...
	public function image() {
        ...
        if((strtolower($ext) == '.zip') || (strtolower($ext) == '.rar')) {
            $this->request->get['image'] = 'zip_icon.png';
        }

        if((strtolower($ext) == '.doc') || (strtolower($ext) == '.docx')) {
            $this->request->get['image'] = 'doc_icon.png';
        }

        if(strtolower($ext) == '.txt') {
            $this->request->get['image'] = 'txt_icon.png';
        }
	...
	public function files() {
		...
		if (!empty($this->request->post['directory'])) {
			...
			$dir = 'data/'.$this->request->post['directory'].'/';
		} else {
			...
			foreach ($files as $file) {
				...
				if (in_array(strtolower($ext), $allowed)) {
					...
					$json[] = array(
						...
						'file_href'=> $this->model_tool_image->resize(html_entity_decode($dir.basename($file), ENT_QUOTES, 'UTF-8'), 100, 100),
		...
		if (isset($this->request->post['directory'])) {
			if (isset($this->request->files['image']) && $this->request->files['image']['tmp_name']) {
				...
				$allowed = array(
                    ...
                    'text/plain', 'application/txt', 'browser/internal', 'text/anytext'
