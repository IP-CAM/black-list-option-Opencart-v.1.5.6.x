	...
	private function _init( $tab ) {
		...
		if( class_exists( 'AceShop' ) && version_compare( $this->config->get('mfilter_aceshop'), $curr_ver, '<' ) ) {
			...
			if( empty( $warnings ) ) {
				...
				$warnings[] = sprintf( 'Remember to make backup your files! <a href="%s">Click here when you done</a>', $this->url->link('module/' . $this->_name . '/aceshop_manually', 'token=' . $this->session->data['token'], 'SSL') );
