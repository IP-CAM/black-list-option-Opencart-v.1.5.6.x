...
var MFP = {
		...
		function selectTab() {
			...
			if( load ) {
				...
				$$.get( '<?php echo $action_get_data; ?>'.replace(/&amp;/g,'&') + MF_AJAX_PARAMS + '&mf_id=' + newId, {}, function( response ){
					...
					$progress.remove();
					showTab( $$.parseJSON( self.base64_decode( $tmp.find('#json-encode').html() ) ) );
				});
			} else {
				showTab({name:''});
			}
	...
	_initModule: function( $module, data ) {
		...
		<?php if( version_compare( VERSION, '1.5.5', '<' ) ) { ?>
			$module.find('[name="mega_filter_module[category_id_with_childs]"]').attr('disabled',true).attr('checked',false).parent().append( ' (This feature is available from OpenCart version >= 1.5.5)');
		<?php } ?>
		...
		<?php if( version_compare( VERSION, '1.5.5', '<' ) ) { ?>
			$module.find('[name="mega_filter_module[hide_category_id_with_childs]"]').attr('disabled',true).attr('checked',false).parent().append( ' (This feature is available from OpenCart version >= 1.5.5)');
		<?php } ?>
				...
				source: function(request, response) {
					jQuery.ajax({
						...
						success: function( res ) {
							var $tmp = $$('<tmp>').html( res ),
								json = $$.parseJSON( self.base64_decode( $tmp.find('#json-encode').html() ) );
