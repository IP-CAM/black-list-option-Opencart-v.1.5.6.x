  ...
  <table class="list" id="images_list">
   <thead>
    <tr>
     ...
     <td class="center">
      <?php if ($product_images['product_options']) { ?>
        <select name="product_images[images_option]">
          <option value="0">Не выбрано</option>
          <?php foreach ($product_images['product_options'] as $option) { ?>
          <?php if ("option[".$option['product_option_id']."]" === $product_images['images_option']) { ?>
   ...
   <?php $image_row = 0; ?>
   <?php foreach ($product_images['product_images'] as $product_image) { ?>
   <tbody id="image-row<?php echo $image_row; ?>">
    <tr>
     ...
     <td class="center"><input name="product_images[product_images][<?php echo $image_row; ?>][sort_order]" type="text" value="<?php echo $product_image['sort_order']; ?>" size="2" /><input id="product_images<?php echo $image_row; ?>" name="product_images[product_images][<?php echo $image_row; ?>][image]" type="hidden" value="<?php echo $product_image['image']; ?>" /></td>
    </tr>
   </tbody>
   <?php $image_row++; ?>
   <?php } ?>
