			...
			jQuery.ajax({
				...
				'success'	: function(response){
					var $tmp = jQuery('<tmp>').html( response ),
						json = jQuery.parseJSON( base64_decode( $tmp.find('#json-encode').html() ) ),
								...
								$cnt2.find('tbody').append(jQuery('<tr>')
									...
									.append(jQuery('<td>')
										.append(jQuery('<div class="image">')
											...
											.append(jQuery('<input type="hidden" name="image[' + json[i].key + ']" value="' + json[i].ival + '" id="image-' + json[i].key + '" />'))
											.append('<br />')
