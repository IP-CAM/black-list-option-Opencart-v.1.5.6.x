...
class ModelCatalogBatchEditorGetLists extends Model {
				...
				if (isset ($option['product_option_value'])) {
					foreach ($option['product_option_value'] as $option_value) {
						$option_value_data[] = array (
							...
							'image_num'				  => $option_value['image_num']
						);
					}
				}
	...
	public function getProductImages($product_id) {
		...
		$q = $this->db->query('SELECT images_option FROM ' . DB_PREFIX . 'product WHERE product_id = ' . (int) $product_id)->row;
		
		$images_option = (isset ($q['images_option'])) ? html_entity_decode ($q['images_option'], ENT_QUOTES, 'UTF-8') : FALSE;
		
		foreach ($product_images as $key => $image) {
			if ($image['image']) {
				$product_images[$key]['thumb'] = $this->model_tool_image->resize($image['image'], 40, 40);
			} else {
				$product_images[$key]['thumb'] = $this->model_tool_image->resize('no_image.jpg', 40, 40);
			}
		}
		
		$images['product_images'] = $product_images;
		$images['product_options'] = $options;
		$images['images_option'] = $images_option;
