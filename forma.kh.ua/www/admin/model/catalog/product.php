...
class ModelCatalogProduct extends Model {
	public function addProduct($data) {
			...
			foreach ($data['product_attribute'] as $product_attribute) {
				if ($product_attribute['attribute_id']) {
					...
					foreach ($product_attribute['product_attribute_description'] as $language_id => $product_attribute_description) {				
						$this->db->query("INSERT INTO " . DB_PREFIX . "product_attribute SET product_id = '" . (int)$product_id . "', attribute_id = '" . (int)$product_attribute['attribute_id'] . "', language_id = '" . (int)$language_id . "', text = '" .  $this->db->escape($product_attribute_description['text']) . "', check_box = '" . $this->db->escape($product_attribute['attr_check']) ."'");
					}
				}
			}
		}
		...
		if (isset($data['images_option'])) {
			$this->db->query("UPDATE " . DB_PREFIX . "product SET images_option = '" . $data['images_option'] . "' WHERE product_id = '" . (int)$product_id . "'");
		}
	...
	public function getProduct($product_id) {
		$query = $this->db->query("SELECT DISTINCT *, (SELECT keyword FROM " . DB_PREFIX . "url_alias WHERE query = 'product_id=" . (int)$product_id . " AND ROWNUM =0') AS keyword FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id) WHERE p.product_id = '" . (int)$product_id . "' AND pd.language_id = '" . (int)$this->config->get('config_language_id') . "'");
...
SELECT pa.attribute_id, ad.name  ,pa.check_box
...
GROUP BY pa.attribute_id");
		...
		foreach ($product_attribute_query->rows as $product_attribute) {
			...
			$product_attribute_data[] = array(
                ...
                'attr_check' =>$product_attribute['check_box'],
			);
		}
			...
			if ($product_option['type'] == 'select' || $product_option['type'] == 'radio' || $product_option['type'] == 'checkbox' || $product_option['type'] == 'image') {
				...
				foreach ($product_option_value_query->rows as $product_option_value) {
					$product_option_value_data[] = array(
						...
						'image_num'				  => $product_option_value['image_num']					
					);
				}
