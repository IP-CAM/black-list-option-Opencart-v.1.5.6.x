...
$this->data['pds_stock'] = $this->language->get('pds_stock');
		...
		if (isset($this->request->get['filter_name']) || isset($this->request->get['filter_tag'])) {
			...
			$keywords = mb_strtolower( $filter_name, 'UTF-8' );
			...
			if( $res ) {
				$dt = ( isset($res->rows) ) ? $res->rows : $res->row;
				foreach( $dt as $key => $values ) {
					$result = $this->model_catalog_product->getProduct($values['product_id']);
					...
					if ($result['ean']>0) {
					if ($result['image']) {
						$image = $this->model_tool_image->resize_watermark($result['image'], $result['ean'], $this->config->get('config_image_product_height'));
						} else {
							$image = false;
						}
					} else {
						if ($result['image']) {
						$image = $this->model_tool_image->resize_watermark($result['image'], $this->config->get('config_image_product_width'), $this->config->get('config_image_product_height'));
					} else {
						$image = false;
					}
					}
...
//tri mod start
        ...
        if (!empty($vip_customer["discount"])) {
          ...
          $special = $this->currency->format($this->tax->calculate($vip_price, $vip_product['tax_class_id'], $this->config->get('config_tax')));
          }
//tri mod end
...
//produc display settings
					...
					'upc'         => $result['upc'],
					'ean'         => ($result['ean'] > 0) ? $result['ean'] : 196,
					'jan'       => $result['jan'],
					'stock'        => 9999,
					//end pds
				...
				if( $res ) {
					...
					foreach( $data as $key => $values ) {
						$data[$key] = array(
							...
							'href' => str_replace("amp;", "", $this->url->link($basehref . $values['product_id'])),
							'image' => $this->model_tool_image->resize($values['image'], 30, 30)
						);
					}
				}
			}
